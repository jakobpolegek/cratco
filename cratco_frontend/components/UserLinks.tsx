          ...
          {links?.length === 0 ? (
            ...
            <ul className="list text-center flex items-center gap-4">
              {links.some((link) => (link.visits || 0) > 0) && (
                <LinkVisitsChart
                  data={links.filter((link) => (link.visits || 0) > 0)}
                />
              )}
              <li className="text-xl uppercase font-semibold text-white opacity-80 tracking-wid">
                My links:
              </li>
              {links?.map((link) => (
                <li
                  className="list-row bg-base-100 rounded-box shadow-md w-4/5 lg:w-auto lg:p-4"
                  key={link._id}
                >
                  <div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          navigator.clipboard.writeText(
                            `${window.location.origin}/${link?.customAddress}`
                          );
                        }}
                      >
