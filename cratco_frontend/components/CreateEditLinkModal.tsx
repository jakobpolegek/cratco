...
const CreateEditLinkModal = forwardRef<
  HTMLDialogElement,
  ICreateEditLinkModalProps
>(({ onClose, editLink = null, mode = 'create' }, ref) => {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setAlert({
          type: 'success',
          message: `${successMessage} This window will automatically close.`,
        });
        setTimeout(() => {
  ...
  return (
    <dialog ref={ref} className="modal">
      <div className="modal-box w-11/12 max-w-xl flex flex-col justify-center items-center">
        ...
        {alert && (
          <div
            className={`alert ${alert.type === 'success' ? 'alert-success' : 'alert-error'} mb-4 w-full`}
          >
        ...
        <div className="flex flex-col items-center w-full">
          ...
          <input
            ...
            onChange={(e) =>
              handleInputChange('originalAddress', e.target.value)
            }
            disabled={isSubmitting}
          />
          {mode === 'edit' && (
          ...
          {mode === 'create' && (
            <p className="py-4 text-center text-sm opacity-70">
              Your links can be viewed by clicking on your name in the
              navigation bar.
            </p>
          )}
        </div>
