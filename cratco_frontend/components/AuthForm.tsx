...
export function AuthForm({ mode }: IAuthFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        `Unexpected error during authentication: : ${err instanceof Error ? err.message : 'An unknown error occurred'}`
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(
        `Google sign in error: ${err instanceof Error ? err.message : 'An unknown error occurred'}`
      );
      setLoading(false);
    }
  };
